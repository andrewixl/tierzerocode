...
class Command(BaseCommand):
    help = 'Sync Microsoft Intune devices automatically'

    def handle(self, *args, **options):
        """Enqueue the Microsoft Intune device sync task"""
        self.stdout.write('Enqueueing Microsoft Intune device sync task...')
        ...
        integration = 'microsoft-intune'
        integration_clean = 'Microsoft Intune'
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'Microsoft Intune device sync task enqueued successfully! Task ID: {result.id}'
                )
            )
            ...
            error_msg = f'Failed to enqueue Microsoft Intune device sync task: {str(e)}'
            self.stdout.write(
