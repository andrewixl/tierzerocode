...
class Command(BaseCommand):
    help = 'Sync Microsoft Defender for Endpoint devices automatically'

    def handle(self, *args, **options):
        """Enqueue the Microsoft Defender for Endpoint device sync task"""
        self.stdout.write('Enqueueing Microsoft Defender for Endpoint device sync task...')
        ...
        integration_clean = 'Microsoft Defender for Endpoint'
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'Microsoft Defender for Endpoint device sync task enqueued successfully! Task ID: {result.id}'
                )
            )
            ...
            error_msg = f'Failed to enqueue Microsoft Defender for Endpoint device sync task: {str(e)}'
            self.stdout.write(
