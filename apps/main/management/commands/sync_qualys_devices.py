...
class Command(BaseCommand):
    help = 'Sync Qualys devices automatically'

    def handle(self, *args, **options):
        """Enqueue the Qualys device sync task"""
        self.stdout.write('Enqueueing Qualys device sync task...')
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'Qualys device sync task enqueued successfully! Task ID: {result.id}'
                )
            )
            ...
            error_msg = f'Failed to enqueue Qualys device sync task: {str(e)}'
            self.stdout.write(
