...
class Command(BaseCommand):
    help = 'Sync Cloudflare Zero Trust devices automatically'

    def handle(self, *args, **options):
        """Enqueue the Cloudflare Zero Trust device sync task"""
        self.stdout.write('Enqueueing Cloudflare Zero Trust device sync task...')
        ...
        integration_clean = 'Cloudflare Zero Trust'
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'Cloudflare Zero Trust device sync task enqueued successfully! Task ID: {result.id}'
                )
            )
            ...
            error_msg = f'Failed to enqueue Cloudflare Zero Trust device sync task: {str(e)}'
            self.stdout.write(
